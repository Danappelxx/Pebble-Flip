...
/*
function actually_send_media_id_list(media_id_list) {
	var media_id_list_length = media_id_list.length;
	
	console.log(media_id_list_length);
}
*/

/*
function send_id_list(media_url_list){
	var temp;
	//var base_url = media_url_list[0].split('?')[0];
	//var access_token = ((media_url_list[0].split('?')[1]).split('&')[1]);
	
	send_app_message({IMAGE_DATA_DL_INPROGRESS: 1});
	
	//send_app_message_with_wait(base_url);
	//send_app_message_with_wait(access_token);
	
	var media_url_list_new = [];
	
	for( var i = 0; i < 10; i++ ) {
		// where temp is the media id
		temp = media_url_list[i];
		temp = 'media_id=' + temp;
		console.log(temp);
		//temp = temp.split('?')[1];
		//temp = temp.split('&')[0];
		media_url_list_new.push(temp);
		send_app_message_with_wait(temp);
	}
	send_app_message({IMAGE_DATA_DL_INPROGRESS: 0});
	
	console.log(media_url_list_new);
	
	// The idea here is that I get the media id from each url and send it seperately
	// Kinda sad since this is the exact opposite of what I wanted to do... hence why it's a list of urls
}
*/

/*
function send_app_message_with_wait(data){
	if(!image_data_message_in_progress){
		setTimeout(function() {
			send_app_message(data);
		}, 1000);
	} else {
		send_app_message_with_wait(data);
	}
}


function send_app_message(data){
	// Make appmessage
	Pebble.sendAppMessage(
		{
			SENDING_IMAGE_DATA: 1,
			IMAGE_DATA: data
		},
		function(e) {
			console.log('Successfully delivered url list');		
